...
export default function Quiz({ moduleId, questions, onComplete }: QuizProps) {
    ...
    return (
      <div className="border border-navy-600 bg-navy-800 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={`font-mono text-2xl ${pct >= 80 ? "text-green-400" : pct >= 60 ? "text-gold-500" : "text-red-400"}`}>
            {pct}%
          </span>
        </div>
        ...
        <div className="space-y-3">
          {questions.map((q, i) => (
            <div key={i} className={`p-3 border ${answers[i] === q.correctIndex ? "border-green-800 bg-green-900/20" : "border-red-800 bg-red-900/20"}`}>
        ...
        <button
          ...
          className="mt-4 px-4 py-2 text-sm bg-navy-700 text-gray-300 hover:bg-navy-600 transition-colors"
        >
          Retake Quiz
        </button>
      </div>
  ...
  return (
    <div className="border border-navy-600 bg-navy-800 p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-white">Module Quiz</h3>
        <span className="font-mono text-sm text-gray-500">
          {currentQ + 1}/{questions.length}
        </span>
      </div>
      <div className="w-full bg-navy-700 h-1 mb-6">
        <div
      ...
      <div className="space-y-2 mb-4">
        {q.options.map((opt, i) => {
          let borderColor = "border-navy-600 hover:border-navy-500";
          if (selected === i && !showResult) borderColor = "border-gold-500";
          if (showResult && i === q.correctIndex) borderColor = "border-green-500 bg-green-900/20";
          if (showResult && selected === i && i !== q.correctIndex) borderColor = "border-red-500 bg-red-900/20";
          ...
          return (
            ...
            >
              <span className="font-mono text-xs text-gray-600 mr-2">{String.fromCharCode(65 + i)}.</span>
      ...
      {showResult && (
        <div className="p-3 bg-navy-700 border border-navy-600 mb-4">
      ...
      <div className="flex justify-end">
        {!showResult ? (
          ...
          <button
            onClick={handleNext}
            className="px-5 py-2 text-sm font-medium bg-gold-500 text-navy-900 hover:bg-gold-400 transition-colors"
          >
